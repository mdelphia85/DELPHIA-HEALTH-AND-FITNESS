...
# JsonStore removed - persistence should be implemented by the backend (DB/API)
...
class WorkoutsScreen(Screen):
    ...
    def _make_row(self, category, exercise_name, inputs=None):
        ...
        ex_field = MDTextField(hint_text=exercise_name, readonly=True)
        self._outline_silver(ex_field)
        ...
        ex_label.bind(
            on_touch_down=lambda l, t, sec=category, f=ex_field:
                self._open_category_menu(sec, f) if l.collide_point(*t.pos) else None
        )

        ex_pair = MDBoxLayout(orientation="vertical", spacing=dp(6), size_hint_x=None, width=dp(220))
        ex_pair.add_widget(ex_field)
        ex_pair.add_widget(ex_label)
        row.add_widget(ex_pair)

        # Input dropdowns (+ equipment)
        widths = {
            "sets": dp(84),
            "reps": dp(84),
            "weight": dp(124),
        ...
        fields = list(inputs if inputs else self.ALL_INPUTS)
        # Always include equipment per row
        if "equipment" not in fields:
            fields.append("equipment")
        
        
        for inp in fields:
            field = MDTextField(hint_text=inp.capitalize(), readonly=True)
            self._outline_silver(field)

            label = MDLabel(
                text=f"[u]{inp.capitalize()}[/u]",
                markup=True,
                theme_text_color="Custom",
                text_color=self.SILVER,
                halign="center",
            ...
            pair = MDBoxLayout(
                ...
                spacing=dp(6),
                size_hint_x=None,
                width=widths.get(inp, dp(110))
            )
            pair.add_widget(field)
            pair.add_widget(label)
            row.add_widget(pair)

            # Bind each label to its correct dropdown (stable per-field capture)
            if inp == "sets":
                label.bind(on_touch_down=lambda l, t, m=self.sets_menu, f=field:
                           self._open_menu(m, f) if l.collide_point(*t.pos) else None)
            elif inp == "reps":
                label.bind(on_touch_down=lambda l, t, m=self.reps_menu, f=field:
                           self._open_menu(m, f) if l.collide_point(*t.pos) else None)
            elif inp == "weight":
                label.bind(on_touch_down=lambda l, t, m=self.weight_menu, f=field:
                           self._open_menu(m, f) if l.collide_point(*t.pos) else None)
            elif inp == "duration":
                label.bind(on_touch_down=lambda l, t, m=self.duration_menu, f=field:
                           self._open_menu(m, f) if l.collide_point(*t.pos) else None)
            elif inp == "distance":
                label.bind(on_touch_down=lambda l, t, m=self.distance_menu, f=field:
                           self._open_menu(m, f) if l.collide_point(*t.pos) else None)
            elif inp == "incline":
                label.bind(on_touch_down=lambda l, t, m=self.incline_menu, f=field:
                           self._open_menu(m, f) if l.collide_point(*t.pos) else None)
            elif inp == "equipment":
                label.bind(on_touch_down=lambda l, t, f=field:
                           self._open_equipment_menu(f) if l.collide_point(*t.pos) else None)

        return row

    # ---------- Dynamic addition ----------
    def _add_new_exercise(self, section, container):
        new_row = self._make_row(section, "Select Exercise", self.ALL_INPUTS)
        # Insert before the "+ Add new exercise" label (which is the last child)
        container.add_widget(new_row, index=len(container.children) - 1)


    def _add_new_exercise(self, section, container):
        new_row = self._make_row(section, "New Exercise", self.ALL_INPUTS)
        container.add_widget(new_row, index=len(container.children) - 1)

    def _make_simple_menu(self, values):
        # Items apply selected value to the caller and close the menu
        menu = MDDropdownMenu(caller=None, items=[])
        try:
            menu.background_color = self.BLACK
            menu.width_mult = 4
        ...
        def _key(self, category):
            """
            Normalize a category name into a safe key string.
            Example: "Recovery & Repair" -> "recovery_repair"
            """
        return (
            category.lower()
            .replace("&", "and")
    ...
    def _apply_dropdown_choice(self, field, value):
        if value in ("+ Add new exercise…", "+ Add new equipment…"):
            if self._active_menu:
                try:
                    self._active_menu.dismiss()
                except Exception:
        ...
        # Footer   <-- 8 spaces (inside the method body)
        footer = MDBoxLayout(size_hint_y=None, height=dp(56), padding=(dp(16), dp(10)), spacing=dp(12))
        footer.add_widget(Widget())
        back = MDLabel(text="[u]Return to Menu[/u]", markup=True,
                       theme_text_color="Custom", text_color=self.SILVER,
...
# ----------------------------
class FreeTrainingScreen(Screen):
    timer_event = None
    start_time = None
    ...
    def on_enter(self):
        ...
        # Workout Name field
        self.ids.workout_name.text_color = gold
        self.ids.workout_name.cursor_color = gold
        self.ids.workout_name.line_color_normal = gold
        self.ids.workout_name.line_color_focus = gold
        self.ids.workout_name.hint_text_color_normal = gold
        self.ids.workout_name.hint_text_color_focus = gold
        self.ids.workout_name.md_bg_color = black

        # Skill Focus field
        self.ids.skill_focus.text_color = gold
        self.ids.skill_focus.cursor_color = gold
        self.ids.skill_focus.line_color_normal = gold
        self.ids.skill_focus.line_color_focus = gold
        self.ids.skill_focus.hint_text_color_normal = gold
        self.ids.skill_focus.hint_text_color_focus = gold
        self.ids.skill_focus.md_bg_color = black

        # Notes field
        self.ids.notes.text_color = gold
        self.ids.notes.cursor_color = gold
        self.ids.notes.line_color_normal = gold
        self.ids.notes.line_color_focus = gold
        self.ids.notes.hint_text_color_normal = gold
        self.ids.notes.hint_text_color_focus = gold
        self.ids.notes.md_bg_color = black
    ...
    def update_timer(self, dt):
        ...
        self.ids.timer_label.text = f"{hrs:02}:{mins:02}:{secs:02}"

    def save_session(self):
        session_data = {
            "workout_name": self.ids.workout_name.text,
            "skill_focus": self.ids.skill_focus.text,
            "notes": self.ids.notes.text,
            "time": self.ids.timer_label.text,
        }
        print("Saved Free Training Session:", session_data)

    def return_to_menu(self):
        self.manager.current = "menu"  # assumes you have a screen named "menu"
...
# ----------------------------
# Goals screen (menus already working)
# ----------------------------
class GoalsScreen(Screen):
    ...
    def on_kv_post(self, base_widget):
        ...
        def make_black_menu(**kwargs):
            menu = MDDropdownMenu(**kwargs)
            def force_black(*_):
                try:
                    for child in menu.walk():
                        if hasattr(child, "md_bg_color"):
                            child.md_bg_color = BLACK
                            break
                except Exception:
                    pass
            menu.bind(on_open=lambda *_: Clock.schedule_once(force_black, 0))
            return menu

        self.visibility_menu = make_black_menu(caller=None, items=[], width_mult=4)
        self.stress_menu = make_black_menu(caller=None, items=[], width_mult=4)
        self.mindfulness_menu = make_black_menu(caller=None, items=[], width_mult=4)
        self.mental_reset_menu = make_black_menu(caller=None, items=[], width_mult=4)
        self.reminder_menu = make_black_menu(caller=None, items=[], width_mult=4)
        self.mood_goal_menu = make_black_menu(caller=None, items=[], width_mult=4)
        self.habits_menu = make_black_menu(caller=None, items=[], width_mult=4)

        self.visibility_menu_items = [
            {"viewclass": "MDLabel", "text": "Private", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("visibility_field", "Private", self.visibility_menu)},
            {"viewclass": "MDLabel", "text": "Show", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("visibility_field", "Show", self.visibility_menu)},
        ]

        self.stress_menu_items = [
            {"viewclass": "MDLabel", "text": "Low", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("stress_field", "Low", self.stress_menu)},
            {"viewclass": "MDLabel", "text": "Moderate", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("stress_field", "Moderate", self.stress_menu)},
            {"viewclass": "MDLabel", "text": "High", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("stress_field", "High", self.stress_menu)},
        ]

        self.mindfulness_menu_items = [
            {"viewclass": "MDLabel", "text": "Meditation", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("mindfulness_field", "Meditation", self.mindfulness_menu)},
            {"viewclass": "MDLabel", "text": "Breathwork", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("mindfulness_field", "Breathwork", self.mindfulness_menu)},
            {"viewclass": "MDLabel", "text": "Journaling", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("mindfulness_field", "Journaling", self.mindfulness_menu)},
        ]

        self.mental_reset_menu_items = [
            {"viewclass": "MDLabel", "text": "Go outside", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("mental_reset_field", "Go outside", self.mental_reset_menu)},
            {"viewclass": "MDLabel", "text": "Say no", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("mental_reset_field", "Say no", self.mental_reset_menu)},
            {"viewclass": "MDLabel", "text": "1hr offline", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("mental_reset_field", "1hr offline", self.mental_reset_menu)},
            {"viewclass": "MDLabel", "text": "Stretch break", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("mental_reset_field", "Stretch break", self.mental_reset_menu)},
        ]

        self.reminder_menu_items = [
            {"viewclass": "MDLabel", "text": "Yes", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("reminder_field", "Yes", self.reminder_menu)},
            {"viewclass": "MDLabel", "text": "No", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("reminder_field", "No", self.reminder_menu)},
        ]

        self.mood_goal_menu_items = [
            {"viewclass": "MDLabel", "text": "Calm", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1)},
            {"viewclass": "MDLabel", "text": "Energized", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1)},
            {"viewclass": "MDLabel", "text": "Focused", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1)},
        ]

        self.habits_menu_items = [
            {"viewclass": "MDLabel", "text": "Sleep 8h", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("habits_field", "Sleep 8h", self.habits_menu)},
            {"viewclass": "MDLabel", "text": "Hydrate", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("habits_field", "Hydrate", self.habits_menu)},
            {"viewclass": "MDLabel", "text": "Walk 30m", "height": dp(48),
             "theme_text_color": "Custom", "text_color": (0.05, 0.2, 0.4, 1),
             "on_release": lambda: self.set_field_and_close("habits_field", "Walk 30m", self.habits_menu)},
        ]

    def set_field_and_close(self, field_id, value, menu):
        field = self.ids.get(field_id)
        if field:
            field.text = value
        menu.dismiss()

    def _apply_text(self, field_id, value):
        field = self.ids.get(field_id)
        if field:
            field.text = value

    def open_visibility_menu(self, caller):
        self.visibility_menu.caller = caller
        self.visibility_menu.items = self.visibility_menu_items
        self.visibility_menu.open()

    def open_stress_menu(self, caller):
        self.stress_menu.caller = caller
        self.stress_menu.items = self.stress_menu_items
        self.stress_menu.open()

    def open_mindfulness_menu(self, caller):
        self.mindfulness_menu.caller = caller
        self.mindfulness_menu.items = self.mindfulness_menu_items
        self.mindfulness_menu.open()

    def open_mental_reset_menu(self, caller):
        self.mental_reset_menu.caller = caller
        self.mental_reset_menu.items = self.mental_reset_menu_items
        self.mental_reset_menu.open()

    def open_reminder_menu(self, caller):
        self.reminder_menu.caller = caller
        self.reminder_menu.items = self.reminder_menu_items
        self.reminder_menu.open()

    def open_mood_goal_menu(self, caller):
        self.mood_goal_menu.caller = caller
        self.mood_goal_menu.items = self.mood_goal_menu_items
        self.mood_goal_menu.open()

    def open_habits_menu(self, caller):
        self.habits_menu.caller = caller
        self.habits_menu.items = self.habits_menu_items
        self.habits_menu.open()

# ----------------------------
# Drawer open helper (used in KV)
# ----------------------------
def open_menu(self):
...
class SupplementsScreen(Screen):
    def on_kv_post(self, base_widget):
        ...
        self.supplements_by_category = {
            "Muscle Growth": [
                "Creatine monohydrate", "Protein powder", "Casein protein", "Beta-alanine",
                "HMB", "Leucine", "BCAA", "Glutamine", "Ecdysterone", "Phosphatidic acid",
            ],
            "Endurance": [
                "Caffeine", "Electrolytes", "Beetroot/nitrate", "Betaine", "Cordyceps",
                "Rhodiola rosea", "Citrulline malate", "Taurine", "Sodium bicarbonate", "Carbohydrate drink",
            ],
            "Recovery & Repair": [
                "Omega-3 (EPA/DHA)", "Collagen peptides", "Tart cherry", "Curcumin", "Vitamin D3",
                "Magnesium glycinate", "Zinc", "Protein powder", "Glucosamine", "Chondroitin",
            ],
            "Strength": [
                "Creatine monohydrate", "Citrulline malate", "Pre-workout (caffeine)", "Ashwagandha",
                "Magnesium glycinate", "Beta-alanine", "HMB", "Ecdysterone", "Phosphatidic acid",
            ],
            "Weight Management": [
                "Green tea extract", "CLA", "L-carnitine", "Protein (satiety)", "Glucomannan",
                "Caffeine", "Garcinia cambogia", "Forskolin", "Fiber supplement",
            ],
        }

        self.unit_options = [
            "g", "mg", "mcg", "IU", "mL", "L", "scoop", "capsule", "tablet", "serving", "per label"
        ]

        self.menus = {}
        self.unit_menus = {}
        self.menu_items = {}
        self.unit_menu_items = {}

        for key, items in self.supplements_by_category.items():
            short = key.lower().replace(" ", "_").replace("&", "and")
            self.menus[short] = make_black_menu(caller=None, items=[], width_mult=4)
            self.unit_menus[short] = make_black_menu(caller=None, items=[], width_mult=4)

            self.menu_items[short] = [
                {
                    "text": name,
                    "theme_text_color": "Custom",
                    "text_color": FOREST_GREEN,
                    "height": dp(48),
                    "on_release": (lambda val=name, m=self.menus[short]: self._apply_and_close(val, m))
                }
                for name in items
            ]

            self.unit_menu_items[short] = [
                {
                    "text": u,
                    "theme_text_color": "Custom",
                    "text_color": FOREST_GREEN,
                    "height": dp(48),
                    "on_release": (lambda val=u, m=self.unit_menus[short]: self._apply_and_close(val, m))
                }
                for u in self.unit_options
            ]

    def _apply_and_close(self, value, menu):
        if menu.caller:
            menu.caller.text = value
        menu.dismiss()

    def open_menu(self, category, caller):
        short = category.lower().replace(" ", "_").replace("&", "and")
        self.menus[short].caller = caller
        self.menus[short].items = self.menu_items[short]
        self.menus[short].open()

    def open_unit_menu(self, category, caller):
        short = category.lower().replace(" ", "_").replace("&", "and")
        self.unit_menus[short].caller = caller
        self.unit_menus[short].items = self.unit_menu_items[short]
        self.unit_menus[short].open()
...
class DelphiaFitnessTrackerApp(MDApp):
    ...
    def set_target_weight_from_text(self, text):
        ...
        try:
            # Store in-memory until backend persistence is available
